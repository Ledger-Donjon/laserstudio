"""
List of the markers present in a viewer, shared by both interfaces.

:class:`MarkersTreeModel` groups markers by label and color. :class:`MarkersView`
is the tree view (with checkboxes for show/hide) embedded either in the classic
window's dock widget or in the new UI's Analyze panel. The model follows
:attr:`~laserstudio.widgets.viewer.Viewer.markers_changed` and refreshes itself.
"""
...
class MarkersTreeModel(QAbstractItemModel):
    ...
    def _sync_marker_visible(self, marker: Marker, visible: bool) -> None:
        if marker.isVisible() == visible:
            return
        marker.setVisible(visible)
        if isinstance(marker, IdMarker):
            ...
            if role == Qt.ItemDataRole.EditRole and index.column() == 2:
                ...
                if new_label != node.marker.label:
                    ...
                    if isinstance(node.marker, IdMarker):
                        self._sync_marker_label(node.marker, new_label)
...
class MarkersView(QTreeView):
    """Tree view over the markers of a viewer, with show/hide checkboxes."""

    def __init__(self, viewer: Viewer, parent: QWidget | None = None):
        super().__init__(parent)
        self.viewer = viewer
        self.markers_model = MarkersTreeModel(viewer, self)
        self.setModel(self.markers_model)
    ...
    def _selected_nodes(self) -> list[_TreeNode]:
        ...
        for index in selection_model.selectedRows(0):
            node = self.markers_model.node_from_index(index)
            if node is None:
                continue
            node_id = id(node)
            if node_id in seen:
        ...
        if isinstance(node, _MarkerNode):
            ...
            if len(marker_nodes) == 1:
                go_action.triggered.connect(
                    lambda: self.center_on(marker_nodes[0])
                )
            menu.addAction(go_action)
            menu.addSeparator()
            menu.addAction("Show", lambda: self.set_visible(marker_nodes, True))
            menu.addAction("Hide", lambda: self.set_visible(marker_nodes, False))
            ...
            if len(marker_nodes) == 1:
                rename_action.triggered.connect(
                    lambda: self.edit(
                        self.markers_model.index_from_node(marker_nodes[0], 2)
                    )
                )
            ...
            if not group_nodes:
                return
            menu.addAction("Show all", lambda: self.set_visible(group_nodes, True))
            menu.addAction("Hide all", lambda: self.set_visible(group_nodes, False))
        else:
            return
