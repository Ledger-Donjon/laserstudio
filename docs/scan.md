...
Scanning zones are organised as a list. Each zone has a name, a colour and an
enabled flag, and can be created, renamed, recoloured, enabled, disabled or
deleted independently.

One zone is *active* at a time. The drawing tools always target the active
...
colour. A disabled zone is not drawn at all, unless it is the active zone — in
...
zone list also holds the name field, the colour swatch, the enable toggle and
...
* `GET /scangeometry/zones` — the zones and the active index
* `POST /scangeometry/zones` — create a zone (`name`, `color`, `enabled`,
  `geometry` are all optional)
* `PATCH /scangeometry/zones/{index}` — update any subset of those fields
* `DELETE /scangeometry/zones/{index}` — delete a zone

Zones are addressed by 0-based index. Colours are returned as `#rrggbb`
strings; `#rrggbbaa` is also accepted on input, but the alpha byte is not
persisted, so it will not survive a later `GET`. `POST` and `PATCH` reject a
malformed `color` or `geometry` with a `400 INVALID_PARAMETER` error. `PUT
/scangeometry`, by contrast, is the same code path used to load
`settings.yaml`: a malformed zone entry there is skipped and logged rather
than rejected, so a bad entry elsewhere in the payload does not prevent the
...
`DELETE /scangeometry` deletes **every** zone, names and colours included — not
...
index = lsapi.add_scan_zone(name="Corner pads", color="#ff5300", enabled=False)["index"]
lsapi.update_scan_zone(index, enabled=True)

# Scan only that zone by disabling the others.
for other in range(len(lsapi.scan_zones()["zones"])):
    if other != index:
        lsapi.update_scan_zone(other, enabled=False)
